...
if ( is_admin() ) {
	add_action( 'admin_enqueue_scripts', 'sentient_load_custom_wp_admin_style' );
	add_action( "admin_enqueue_scripts", 'optionsframework_mlu_js', 0 );
}

function sentient_load_custom_wp_admin_style() {
	wp_enqueue_style( 'optionsframework_mlu_css' );
}
...
function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
	...
	$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . esc_html__( 'Upload', 'selfie' ) . '" rel="' . $int . '" />' . "\n";
	...
	if ( $value != '' ) { 
		...
		if ( $image ) {
			...
			$title = esc_html__( 'View File', 'selfie' );
...
function optionsframework_mlu_get_silentpost ( $_token ) {
	...
	if ( $_token && (is_object($selfieWP))) {
		
		// Tell the function what to look for in a post.
		
		
		if ( post_type_exists( 'optionsframework' ) ) {
		...
		if(is_array($_args)){
			...
			$query = 'SELECT ID FROM ' . $selfieWP->posts . ' WHERE post_parent = 0';
			foreach ( $_args as $k => $v ) {
				$query .= ' AND ' . $k . ' = "' . $v . '"';
			} // End FOREACH Loop
			
			$query .= ' LIMIT 1';
			$_posts = $selfieWP->get_row( $query );
...
function optionsframework_mlu_modify_tabs ( $tabs ) {
	$tabs['gallery'] = str_replace( esc_html__( 'Gallery', 'selfie' ), esc_html__( 'Previously Uploaded', 'selfie' ), $tabs['gallery'] );
	return $tabs;
}
