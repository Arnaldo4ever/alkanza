...
class Radium_Theme_Importer {
    ...
    public function add_admin() {
		...
		add_theme_page(esc_html__('Import Demo Data', 'selfie'), esc_html__('Import Demo Data', 'selfie'), 'switch_themes', 'radium_demo_installer', array($this, 'demo_installer'));
    ...
    public function set_demo_data( $file ) {
        ...
        if ( !class_exists( 'WP_Importer' ) ) {
            ...
            if ( file_exists( $class_wp_importer ) ){
				get_template_part( 'class', 'wp-importer' );
            } else {
        ...
        if ( !class_exists( 'WP_Import' ) ) {
			...
			$class_wp_import = get_template_directory() .'/importer/importer/wordpress-importer.php';
            
			if ( file_exists( $class_wp_import ) ) 
				get_template_part( 'importer/importer/wordpress', 'importer' );
            else
                $importer_error = true;
    ...
    public function set_demo_theme_options( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			wp_die(
				esc_html__( 'Theme options Import file could not be found. Please try again.', 'selfie' ),
				'',
		...
		$getdata = wp_remote_get($file);
		
		if( is_array($getdata) ) {
		  $data = unserialize( trim($getdata['body'], '###') );
		}	
		...
		if ( empty( $data ) || ! is_array( $data ) ) {
			wp_die(
				esc_html__( 'Theme options import data could not be read. Please try a different file.', 'selfie' ),
				'',
				array( 'back_link' => true )
			);
		}
    ...
    function process_widget_import_file( $file ) {
    	...
    	if ( ! file_exists( $file ) ) {
    		wp_die(
    			esc_html__( 'Widget Import file could not be found. Please try again.', 'selfie' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    ...
    public function import_widgets( $data ) {
    	...
    	if ( empty( $data ) || ! is_object( $data ) ) {
    		wp_die(
    			esc_html__( 'Widget import data could not be read. Please try a different file.', 'selfie' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    	...
    	foreach ( $data as $sidebar_id => $widgets ) {
    		...
    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
    			...
    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'selfie' );
    		}
    		...
    		foreach ( $widgets as $widget_instance_id => $widget ) {
    			...
    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
    				...
    				$widget_message = esc_html__( 'Site does not support widget', 'selfie' ); // explain why widget not imported
    			}
    			...
    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
    				...
    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
    					...
    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
    						...
    						$widget_message = esc_html__( 'Widget already exists', 'selfie' ); // explain why widget not imported
    			...
    			if ( ! $fail ) {
    				...
    				if ( $sidebar_available ) {
    					...
    					$widget_message = esc_html__( 'Imported', 'selfie' );
    				} else {
    					$widget_message_type = 'warning';
    					$widget_message = esc_html__( 'Imported to Inactive', 'selfie' );
    				}
    			...
    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'selfie' ); // show "No Title" if widget instance is untitled
